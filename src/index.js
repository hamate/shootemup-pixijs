...
function checkShipCollision(ship) {
  let objectBox = ship.getBounds();
  let objectBorders = {
    objectTail: objectBox.left,
    objectHead: objectBox.right,
    objectTop: objectBox.top,
    objectBot: objectBox.bottom,
  };
  enemies.forEach((enemy, i) => {
    let enemyBox = enemy.getBounds();
    let enemyBorders = {
      enemyTail: enemyBox.right,
      enemyHead: enemyBox.left,
      enemyTop: enemyBox.top,
      enemyBot: enemyBox.bottom,
    };

    if (enemyBorders.enemyHead < objectBorders.objectHead) {
      if (
        enemyBorders.enemyTop < objectBorders.objectBot &&
        enemyBorders.enemyBot > objectBorders.objectBot
      ) {
        console.log('obj1: ', ship);
        console.log('enemy1: ', enemy);
        explodeObject(ship);
        state = endGame;
      }
      if (
        enemyBorders.enemyBot > objectBorders.objectTop &&
        enemyBorders.enemyTop < objectBorders.objectTop
      ) {
        console.log('obj2: ', ship);
